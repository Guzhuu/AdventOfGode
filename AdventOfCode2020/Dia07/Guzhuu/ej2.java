	...
	public static void main(String[] args) {
		...
		try{
			...
			for(Map.Entry<Bag, List<Bag>> entry : bags.entrySet()) {
				...
				if(key.getColor().indexOf("shiny gold") != -1){
					count = contarBolsas(key, value);
				}
			}
			System.out.println("Num bolsas: " + (count-1)); //Se cuenta la shiny gold de más
		}catch(Exception e){
			System.out.println("Oh no: " + e.toString());
		}
	}

	public static int contarBolsas(Bag bag, List<Bag> list){ //Se puede mejorar guardando un array de colores que contienen shiny bags
		int count = 0;
		if(list.isEmpty()){
			return 1;
		}else{
			for(int i = 0; i < list.size(); i++){
				...
				count += (list.get(i).getCount() * contarBolsas(list.get(i), bags.get(bag2)));
			}
		}
		return count+1;
	}
